      ...
      <x-slot name="filters">
        <form action="{{ route('admin.users.index') }}" method="get" class="flex flex-col gap-2 md:flex-row">
          <div class="relative flex-1">
            <input type="search" name="search" class="form-control peer ps-11 shadow-sm" placeholder="Cari group ...">
      ...
      @foreach ($groups as $i => $group)
        <tr
          ...
          <td class="whitespace-nowrap flex gap-2 px-6 py-4 text-end text-sm font-medium">
            <a href="{{ route('admin.groups.edit', $group->id) }}" class="btn bg-caution text-white flex items-center gap-2">
            ...
            <button type="button" class="btn bg-danger text-white flex items-center gap-2"
              hx-confirm="group" hx-headers='{"X-CSRF-TOKEN": "{{ csrf_token() }}"}'

...
@section('content')
  <x-dashboard-layout active="mapel">
    <x-data-table :columns="$columns" :routes="$routes" :title="$title">
      <x-slot name="filters">
        <form action="{{ route('admin.subjects.index') }}" method="get" class="flex flex-col gap-2 md:flex-row">
          <div class="relative flex-1">
            <input type="search" name="search" class="form-control peer ps-11 shadow-sm" placeholder="Cari mapel ...">
      ...
      @foreach ($subjects as $i => $subject)
        <tr
          ...
          <td class="whitespace-nowrap flex gap-2 px-6 py-4 text-end text-sm font-medium">
            <a href="{{ route('admin.subjects.edit', $subject->id) }}" class="btn bg-caution text-white flex items-center gap-2">
            ...
            <button type="button" class="btn bg-danger text-white flex items-center gap-2"
              hx-confirm="mapel" hx-headers='{"X-CSRF-TOKEN": "{{ csrf_token() }}"}'
      ...
      <x-slot name="footer">
        <div class="flex justify-between">
          <x-paginate-links :links="$subjects"></x-paginate-links>

          <x-paginate-counter :items="$subjects"></x-paginate-counter>
        </div>
      </x-slot>
    </x-data-table>
  </x-dashboard-layout>
